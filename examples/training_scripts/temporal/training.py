...
train_dataset = dataset[:int(training_fraction*total_samples)]
...
val_loader = DataLoader(valset, batch_size=1000, shuffle=False)
...
test_scores = []
with torch.no_grad():
    for val_batch_0, val_batch_1 in test_loader:
        model.validate(
            (
                val_batch_0.to(device=model._device),
                val_batch_1.to(device=model._device),
            )
        )

    mean_score = model._estimator.output_mean_score()
    test_scores.append(mean_score)

print('Test scores:', test_scores)

try:
    model._test_scores = test_scores
except AttributeError:
    print('Test scores not saved')

# save model object
torch.save(model, os.path.join(save_path, 'model_object.pt'))
